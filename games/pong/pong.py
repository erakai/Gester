...
class Ball(Rect):
    ...
    def __init__(self, *args):
        super().__init__()
        sounds.create_sound("ball", "games/assets/sounds/pong-ball.wav")
    ...
    def on_collide(self, obj):
        ...
        if isinstance(obj, Enemy):
            if x > obj.position.get_x() + obj.size.get_width():
                self.speed_y = -self.speed_y
            else:
                self.speed_x = -self.speed_x
            sounds.play_sound("ball")
...
def check_ball_collide(ball: Ball, obj: Rect):
    ...
    return (
        r1x + r1w >= r2x and r1x <= r2x + r2w and r1y + r1h >= r2y and r1y <= r2y + r2h
    )
...
gameBound = GameBounds(
    ball, player, enemy, top_bound, bottom_bound, player_score, enemy_score
)
